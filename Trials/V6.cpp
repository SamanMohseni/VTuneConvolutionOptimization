...
void random_fill_matrix_with_zero_padding(double** matrix, int height, int width, int padding) {
	for (int i = 0; i < height; i++) {
...
void parallel_random_fill_matrix_with_zero_padding(double** matrix, int height, int width, int padding) {
	unsigned int thread_count = thread::hardware_concurrency();
	thread* threads = new thread[thread_count];

	for (int i = 0; i < thread_count; i++) {
		threads[i] = thread(random_fill_matrix_with_zero_padding, matrix + i * (height / thread_count), height / thread_count, width, padding);
	}
...
int main() {
	...
	int padding = 4 - filter_width % 4; // pad to make filter multiple of 4
	...
	random_fill_matrix_with_zero_padding(filter, filter_height, filter_width, padding);
	creat_matrix(source, source_height, source_width + padding);
	parallel_random_fill_matrix_with_zero_padding(source, source_height, source_width, padding);
